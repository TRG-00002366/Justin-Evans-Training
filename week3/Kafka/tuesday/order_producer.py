...
def generate_order(order_number: int) -> dict:
    ...
    if order_number / 100:
        pass
    if order_number / 10:
        order_num_formatted = "0" + str(order_number)
    else:
        order_num_formatted = "00" + str(order_number)

    product, price = products[random.randint(0,4)]
    
    order = {
        "order_id": "ORD-" + order_num_formatted,
        "customer_id": "CUST" + str(random.randint(100, 999)),
        "product": product,
