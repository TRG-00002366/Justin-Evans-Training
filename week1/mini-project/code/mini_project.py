...
stopwords = sc.broadcast(["the", "a", "an", "is", "of", "and"])
print(f"Word count before removing stop words: {tokenized_book.count()}")

remove_stopwords = book_in_lowercase \
    .flatMap(word_splitting) \
    .filter(lambda word: word not in stopwords.value)

print(f"Word count after removing stop words: {remove_stopwords.count()}")
print(f"First 250 words: {remove_stopwords.take(250)}")
...
#Start of task 12
print("Task 12")
chapter_titles_bc = sc.broadcast(["ADVENTURE I. A SCANDAL IN BOHEMIA", "The Red-headed League"])
chapter_index_checks = indexed_rdd.filter(lambda line: line[1] < 892)
print(f"Some of the First Chapter: {chapter_index_checks.take(50)}")
#Use the print statement below to get the whole chapter
#print(f"Full First Chapter: {chapter_index_checks.collect()}")
