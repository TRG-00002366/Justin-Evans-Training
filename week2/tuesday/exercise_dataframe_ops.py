...
df.withColumn("country", lit("USA")).select("*").show()
...
df.withColumn("name_upper", upper(col("name"))).select("*").show()


# TODO 3d: Modify salary to increase by 5% (replace the column)

df.withColumn("salary", col("salary")*1.05).select("*").show()
...
df.filter(col("salary") > 70000).select("*").show()


# TODO 4b: Filter employees in Engineering department

df.filter(col("department") == "Engineering").select("*").show()

# TODO 4c: Filter employees in Engineering OR Marketing

df.filter((col("department") == "Engineering") | (col("department") ==  "Marketing")).select("*").show()

# TODO 4d: Filter employees hired after 2020-01-01 with salary > 60000
# HINT: You can compare date strings directly

df.filter((col("hire_date") > "2020-01-01") & (col("salary") < 60000)).select("*").show()
