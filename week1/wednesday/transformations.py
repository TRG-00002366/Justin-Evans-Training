...
errors = logs.filter(lambda log: re.search("ERROR", log))
...
error_messages = logs \
.filter(lambda log: re.search("ERROR", log)) \
.map(lambda x: x.split("ERROR ")[1])
...
page_path = web_logs \
    .filter(lambda log: log.split()[3] == "200") \
    .map(lambda log: (log.split()[2], 1)) \
    .reduceByKey(lambda a, b: a + b) \
    .sortBy(lambda a: a[1], ascending=False)
print(f"Page path: {page_path.collect()}")
